...
if (ch1 == 0) {
    ...
    for (j = 0; j < r; j++) scanf("%d", &request[j]);
    for (i = 0; i < p; i++) {
    if (i == new) {
        for (j = 0; j < r; j++) {
        if (request[j] > need[i][j]) violationcheck = 1;
        if (request[j] > avail[j]) waitcheck = 1;
        }
        if (violationcheck == 1) {
...
for (j = 0; j < p; j++) completed[j] = 0;
for (tk = 0; tk < r; tk++) {
    for (j = 0; j < p; j++) {
    if (completed[j] == 0) {
        check1 = 0;
        for (k = 0; k < r; k++)
        if (need[j][k] <= work[k]) check1++;
        if (check1 == r) {
        for (k = 0; k < r; k++) {
...
check2 = 0;
for (i = 0; i < p; i++)
    if (completed[i] == 1) check2++;
printf("\n");
if (check2 >= p) {
    printf("\nThe System is in Safe State\n");
    for (tj = 0; tj < p; tj++) printf("p%d", sseq[tj]);
    return 1;
} else {

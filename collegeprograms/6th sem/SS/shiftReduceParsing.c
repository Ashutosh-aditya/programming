...
while (1) {
    ...
    if (stack[sptr] == 'T' && stack[sptr - 1] == '+' &&
        stack[sptr - 2] == 'E' && ip[iptr] != '*') {
    // stack[sptr-2]='E';
    stack[sptr - 1] = '\0';
    sptr = sptr - 2;
    flag = 1;
    printf("\n $%s\t\t%s$\t\tE->E+T", stack, ip);
    } else if ((stack[sptr] == 'T' && ip[iptr] == '+') ||
            (stack[0] == 'T' && ip[iptr] == '\0') ||
            (stack[sptr] == 'T' && ip[iptr] == ')')) {
    stack[sptr] = 'E';
    flag = 1;
    printf("\n $%s\t\t%s$\t\tE->T", stack, ip);
    }
